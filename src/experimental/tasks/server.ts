...
> {
    ...
    createMessageStream(
        ...
        if (params.messages.length > 0) {
            ...
            if (hasPreviousToolUse) {
                ...
                const toolUseIds = new Set(previousContent.filter(c => c.type === 'tool_use').map(c => (c as { id: string }).id));
                const toolResultIds = new Set(
                    lastContent.filter(c => c.type === 'tool_result').map(c => (c as { toolUseId: string }).toolUseId)
                );
    ...
    elicitInputStream(
        ...
        const normalizedParams = mode === 'form' && params.mode === undefined ? { ...params, mode: 'form' as const } : params;
